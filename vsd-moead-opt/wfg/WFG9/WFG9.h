 ...
 *
 * DESCRIPCION
 *    Individuo para la resolucion del problema de Benchmark WFG9
 *    
 *    El problema WFG9 esta definido en:
...
#ifndef __INDIVIDUALWFG9_H__
#define __INDIVIDUALWFG9_H__
...
//	unsigned int inline getOptDirection(const int i) const { return MINIMIZE; }
  ...
  // Superficies genericas
	...
	// Transiciones del problema WFG9
	vector<double> WFG9_t1(const vector<double> &y);
	vector<double> WFG9_t2(const vector<double> &y, const int k);
	vector<double> WFG9_t3(const vector<double> &y, const int k, const int M);
	
	// Superficie del problema WFG9
	vector<double> WFG9_shape(const vector<double> &t_p);
