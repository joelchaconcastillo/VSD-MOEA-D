 ...
 *
 * DESCRIPCION
 *    Individuo para la resolucion del problema de Benchmark WFG7
 *    
 *    El problema WFG7 esta definido en:
...
#ifndef __INDIVIDUALWFG7_H__
#define __INDIVIDUALWFG7_H__
...
//	unsigned int inline getOptDirection(const int i) const { return MINIMIZE; }
  ...
  // Superficies genericas
	...
	// Transiciones del problema WFG7
	vector<double> WFG7_t1(const vector<double> &y, const int k);
	vector<double> WFG7_t2(const vector<double> &y, const int k);
	vector<double> WFG7_t3(const vector<double> &y, const int k, const int M);
	
	// Superficie del problema WFG7
	vector<double> WFG7_shape(const vector<double> &t_p);
