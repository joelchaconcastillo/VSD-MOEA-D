 ...
 *
 * DESCRIPCION
 *    Individuo para la resolucion del problema de Benchmark WFG6
 *    
 *    El problema WFG6 esta definido en:
...
#ifndef __INDIVIDUALWFG6_H__
#define __INDIVIDUALWFG6_H__
...
//	unsigned int inline getOptDirection(const int i) const { return MINIMIZE; }
  ...
  // Superficies genericas
	...
	// Transiciones del problema WFG6
	vector<double> WFG6_t1(const vector<double> &y, const int k);
	vector<double> WFG6_t2(const vector<double> &y, const int k, const int M);
	
	// Superficie del problema WFG6
	vector<double> WFG6_shape(const vector<double> &t_p);
